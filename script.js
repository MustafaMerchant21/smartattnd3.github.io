...
function locationAccessedAlert(status){
  setTimeout(() => {
    document.getElementById("locationAlert").style.display = "block";
    document.getElementById("locationAlert").innerHTML = status;
  }, 1500);
  setTimeout(() => {
  ...
  if ("geolocation" in navigator) {
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        initMap();
        locationAccessedAlert('Successfully accessed the location')
      },
      (error) => {
        alert(error.message + ". Try enabling your location.");
        locationAccessedAlert('Failed accessing the location')
      },
      {
        maximumAge:0,
  ...
  function checkLocation() {
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          aalert(error.message + ". Try enabling your location.");
          locationAccessedAlert('Failed accessing the location');
        },
        {maximumAge:0}
      );
